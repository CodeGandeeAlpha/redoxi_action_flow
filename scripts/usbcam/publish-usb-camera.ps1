...
$command = if ($PROTOCOL -eq "rtsp") {
    "ffmpeg -f dshow -i video=`"$USB_NAME`" -fflags nobuffer -flags low_delay -preset ultrafast -vcodec libx264 -tune zerolatency -b 6000k -f rtsp $outputUrl"
} elseif ($PROTOCOL -eq "rtp") {
    "ffmpeg -f dshow -i video=`"$USB_NAME`" -fflags nobuffer -flags low_delay -preset ultrafast -vcodec libx264 -tune zerolatency -b 6000k -f rtp $outputUrl"
} elseif ($PROTOCOL -eq "tcp") {
    "ffmpeg -f dshow -i video=`"$USB_NAME`" -fflags nobuffer -flags low_delay -preset ultrafast -vcodec libx264 -tune zerolatency -b 6000k -f mpegts $outputUrl"
} else {
    "ffmpeg -f dshow -i video=`"$USB_NAME`" -fflags nobuffer -flags low_delay -preset ultrafast -vcodec libx264 -tune zerolatency -b 6000k -f mpegts $outputUrl"
}
