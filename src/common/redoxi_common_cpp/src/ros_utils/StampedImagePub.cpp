...
StampedImagePub::StampedImagePub(rclcpp::Node *node, const std::string &topic_name, int queue_size)
    : m_node(node)
{
                          ...
                          int queue_size)
{
    ...
    m_node = node;
    m_pub = node->create_publisher<sensor_msgs::msg::Image>(topic_name, queue_size);
