                     ...
                     const DrawDetectionsOptions &options)
{
    ...
    //! Convert detections to ROS2 message using image utils
    std::vector<redoxi_public_msgs::msg::Detection> ros_detections;
    inference::conversion::to_ros_msg(&ros_detections, detections);

    //! Use image utils to draw detections on the image
    redoxi_works::image_utils::draw_detections(output, ros_detections, options);
