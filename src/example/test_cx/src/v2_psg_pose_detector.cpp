...
int main(int argc, char **argv)
{
    ...
    init_config->parse_from_node_parameters(init_config.get(), psg_pose_detector_node.get());
    {
        RDX_INFO_DEV(psg_pose_detector_node.get(), __func__, false, "{}", "Converting init config to JSON");
        auto init_config_json = JS::serializeStruct(*init_config);
        RDX_INFO_DEV(psg_pose_detector_node.get(), __func__, false, "InitConfig JSON: {}", init_config_json);
    }
    auto runtime_config = std::make_shared<rdx::PSGPoseDetectorNode::RuntimeConfig_t>();
    runtime_config->parse_from_node_parameters(runtime_config.get(), psg_pose_detector_node.get());
    {
        RDX_INFO_DEV(psg_pose_detector_node.get(), __func__, false, "{}", "Converting runtime config to JSON");
        auto runtime_config_json = JS::serializeStruct(*runtime_config);
        RDX_INFO_DEV(psg_pose_detector_node.get(), __func__, false, "RuntimeConfig JSON: {}", runtime_config_json);
    }
