    ...
    if simplejpeg:
        if len(img.shape) == 2:
            ...
            return simplejpeg.encode_jpeg(
                img, colorspace="GRAY", quality=DefaultSettings.JpegQuality
            )
            ...
            if img.shape[2] == 1:
                ...
                return simplejpeg.encode_jpeg(
                    img, colorspace="RGBA", quality=DefaultSettings.JpegQuality
                )
            elif img.shape[2] == 3:
                return simplejpeg.encode_jpeg(
                    img, colorspace="RGB", quality=DefaultSettings.JpegQuality
                )
        else:
            return b""
    elif cv2:
        if len(img.shape) == 3 and img.shape[2] == 3:
            img = img[:, :, ::-1]
        return cv2.imencode(
            ".jpg", img, [cv2.IMWRITE_JPEG_QUALITY, DefaultSettings.JpegQuality]
        )[1].tobytes()
    elif PIL:
        pil_img = Image.fromarray(img)
        buffered = io.BytesIO()
        pil_img.save(buffered, format="JPEG", quality=DefaultSettings.JpegQuality)
...
def compress_compressed_image(msg, output):
    ...
    try:
        ...
        if (
            img.shape[0] > DefaultSettings.MaxImageSize
            or img.shape[1] > DefaultSettings.MaxImageSize
        ):
            stride = int(
                np.ceil(
                    max(
                        img.shape[0] / float(DefaultSettings.MaxImageSize),
                        img.shape[1] / float(DefaultSettings.MaxImageSize),
                    )
                )
            )
...
def compress_image(msg, output):
    ...
    if (
        cv2_img.shape[0] > DefaultSettings.MaxImageSize
        or cv2_img.shape[1] > DefaultSettings.MaxImageSize
    ):
        stride = int(
            np.ceil(
                max(
                    cv2_img.shape[0] / float(DefaultSettings.MaxImageSize),
                    cv2_img.shape[1] / float(DefaultSettings.MaxImageSize),
                )
            )
        )
...
def compress_occupancy_grid(msg, output):
    ...
    try:
        ...
        while (
            occupancy_map.shape[0] > DefaultSettings.MaxImageSize
            or occupancy_map.shape[1] > DefaultSettings.MaxImageSize
        ):
            occupancy_map = occupancy_map[::2, ::2]

        cv2_img = ((100 - occupancy_map) * 10 // 4).astype(
            np.uint8
