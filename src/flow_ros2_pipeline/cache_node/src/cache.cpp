...
namespace FlowRos2Pipeline {
    CacheNode::CacheNode() : rclcpp::Node("cache_node"), logger_(this->get_logger()) {
        ...
        // 创建service server
        // m_server_register_obj_id = this->create_service<psg_services::srv::RegisterObjId>(register_obj_id_srv, &CacheNode::register_obj_id);
        // m_server_get_obj_id = this->create_service<psg_services::srv::GetObjId>(get_obj_id_srv, &CacheNode::get_obj_id);
        // m_server_delete_obj_id = this->create_service<psg_services::srv::DeleteObjId>(delete_obj_id_srv, &CacheNode::delete_obj_id);

        RCLCPP_INFO(logger_, "[CacheNode] init success!");
    }

    // void CacheNode::register_obj_id(const std::shared_ptr<psg_services::srv::RegisterObjId::Request> request,
    ...
    // void CacheNode::get_obj_id(const std::shared_ptr<psg_services::srv::GetObjId::Request> request,
    //             std::shared_ptr<psg_services::srv::GetObjId::Response> response) {
    //     response->obj_id = "";
    ...
    // void CacheNode::delete_obj_id(const std::shared_ptr<psg_services::srv::GetObjId::Request> request,
    //             std::shared_ptr<psg_services::srv::GetObjId::Response> response) {
    //     response->obj_id = "";
    //     response->flag = "obj not exist";
    //     response->return_code = 1;
