...
class PSGCount : public rclcpp::Node, public IStartStopProtocol
{
  public:
  ...
  public:
    ...
    using InitConfig = PSGCountInitConfig;
    using RuntimeConfig = PSGCountRuntimeConfig;
    using MSG_PsgDocument = psg_private_msgs::msg::PsgDocument;
    using MSG_TrajectoryEvents = psg_private_msgs::msg::TrajectoryEvents;
    ...
    using Map_Documents = std::map<int, MSG_PsgDocument>;
  ...
  protected:
    ...
    // add buffer
    virtual void _add_document_to_buffer(const MSG_PsgDocument &document, Map_Documents *document_buffer_ptr);

    // remove buffer
    virtual void _remove_document_from_buffer(int frame_number, Map_Documents *document_buffer_ptr);
  ...
  protected:
    ...
    // impl data
    std::shared_ptr<PSGCountImpl> m_impl;

    // buffer for detections
    Map_Documents m_documents_buffer;
