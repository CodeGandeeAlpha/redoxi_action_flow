...
#include <tracker/_pipeline_in.hpp>
...
{
TrackerIn::TrackerIn()
    : Node("tracker_in_node")
{
    m_impl = std::make_shared<TrackerInImpl>(this);
...
int TrackerIn::init(const std::shared_ptr<InitConfig> &config,
                    const std::shared_ptr<RuntimeConfig> &runtime_config)
{
    ...
    m_act_process_document = rclcpp_action::create_server<ACT_AcceptDocument>(
        ...
        std::bind(&TrackerIn::_accept_document_goal_callback, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&TrackerIn::_accept_document_cancel_callback, this, std::placeholders::_1),
        std::bind(&TrackerIn::_accept_document_accepted_callback, this, std::placeholders::_1));
...
const std::shared_ptr<TrackerIn::InitConfig> &TrackerIn::get_init_config() const
{
    return m_init_config;
}

int TrackerIn::update_runtime_config(const std::shared_ptr<RuntimeConfig> &config)
{
...
const std::shared_ptr<TrackerIn::RuntimeConfig> &TrackerIn::get_runtime_config() const
{
...
int TrackerIn::get_status_code() const
{
...
rclcpp_action::CancelResponse TrackerIn::_accept_document_cancel_callback(
...
void TrackerIn::_accept_document_accepted_callback(
    const std::shared_ptr<rclcpp_action::ServerGoalHandle<ACT_AcceptDocument>> goal_handle)
{
    ...
    // time log
    RCLCPP_INFO(m_impl->logger, "---TIME LOG: framenum %ld node %s type %s time %ld", goal->document.frame.frame_num, "tracker", "IN", this->now().nanoseconds());
...
void TrackerIn::_process_document_create_tasks(const MSG_PsgDocument &document, Map_Document_Waiting *psgdoc_task_waiting_ptr)
{
...
void TrackerIn::_connect_to_downstreams()
{
    ...
    for (auto it : m_init_config->pipeline_downstreams) {
        ...
        // create pipeline downstream
        {
            ...
            //         std::bind(&TrackerIn::process_document_goal_response_callback, this, std::placeholders::_1);
            // ds->accept_document_options.feedback_callback =
            //         std::bind(&TrackerIn::process_document_feedback_callback, this, std::placeholders::_1, std::placeholders::_2);
            // ds->accept_document_options.result_callback =
            //         std::bind(&TrackerIn::process_document_result_callback, this, std::placeholders::_1);
...
void TrackerIn::_send_detections_to_downstreams()
{
...
void TrackerIn::_send_document_to_downstreams()
{
...
void TrackerIn::_declare_all_parameters()
{
