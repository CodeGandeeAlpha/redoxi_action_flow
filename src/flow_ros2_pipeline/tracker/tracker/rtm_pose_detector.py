from tracker.base_pose_detector import BasePoseDetector, PoseDetectionResult
...
class RTMPoseDetector(BasePoseDetector, BaseTool):
    ...
    def init(self,
                 ...
                 device: str = 'cpu'):
        BaseTool.__init__(self, onnx_model, model_input_size, mean, std, backend, device)
    ...
    def __call__(self, image: np.ndarray, bboxes: list = []):
        return self.infer(image, bboxes)
    ...
    def infer(self, image: np.ndarray, bboxes: list = []):
        ...
        result = PoseDetectionResult(keypoints=keypoints, scores=scores)
        return result