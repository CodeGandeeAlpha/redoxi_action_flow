...
class StampedFramePub
{
  public:
    using MessageType_t = redoxi_public_msgs::msg::Frame;
    ...
    //! Constructor
    StampedFramePub() = default;

    //! Constructor that creates publisher during construction
    StampedFramePub(rclcpp::Node *node, const std::string &topic_name, std::optional<rclcpp::QoS> qos = std::nullopt);
