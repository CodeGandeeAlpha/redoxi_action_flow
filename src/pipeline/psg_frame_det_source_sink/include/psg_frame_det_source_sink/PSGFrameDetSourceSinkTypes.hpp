...
{
class PSGFrameDetSourceSink;

namespace psg_frame_det_source_sink
{
using OutputPortType = AsyncGetDetectionsOutputPort;
...
struct InitConfig : public common_nodes::StartStopNode::InitConfig_t {
    ...
    std::shared_ptr<OutputPortSpec::InitConfig_t> primary_output_spec;
    ...
    //! parse from node, the node must be exactly PSGFrameDetSourceSink, not its subclass
    template <typename Node_t>
    requires std::is_same_v<Node_t, PSGFrameDetSourceSink>
